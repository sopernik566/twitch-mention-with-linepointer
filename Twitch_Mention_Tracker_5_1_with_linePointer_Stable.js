// ==UserScript==
// @name         Twitch_Mention_Tracker_with_linePointer
...
(function () {
    ...
    // Создание указателя (палочки) для каждого сообщения
const createLinePointer = (parentElement, top, left, rotation, color) => {
    const linePointer = document.createElement('div');
    linePointer.style.position = 'relative';
    linePointer.style.top = '25px';
    linePointer.style.left = '18px';
    linePointer.style.width = '379px';
    linePointer.style.height = '3px';
    linePointer.style.backgroundColor = '#b86e01';
    linePointer.style.transform = `rotate(177deg)`;
    linePointer.style.display = 'none'; // По умолчанию скрыт
    linePointer.style.zIndex = '1';
    linePointer.style.borderRadius = '25px';
    parentElement.appendChild(linePointer);
    return linePointer;
};

// Изменение функции createToggleButton для управления указателем
const createToggleButton = (nickname, isCollapsed, toggleFunction, linePointer) => {
    ...
    toggleButton.addEventListener('click', () => {
        ...
        // Управление видимостью указателя
        if (linePointer) {
            linePointer.style.display = isCollapsed ? 'none' : 'block';
        }
...
// Обновление списка упоминаний с добавлением указателя для каждого сообщения
const updateMentionList = () => {
    ...
    Object.keys(mentions).forEach((nickname, index) => {
        ...
        // Создание указателя для каждого сообщения
        const linePointer = createLinePointer(
            mentionItem,
            `${50 + index * 60}px`, // Позиция по вертикали для каждого сообщения
            '10px', // Позиция по горизонтали
            45, // Угол поворота
            '#611678' // Цвет
        );
